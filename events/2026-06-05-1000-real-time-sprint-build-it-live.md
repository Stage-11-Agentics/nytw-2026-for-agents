---
title: "Real-Time Sprint: Build It Live"
event_id: "hy44NsXy8fICdvIMp2bI"
date: 2026-06-05
day: "Friday"
start_time: "10:00am ET"
end_time: "2:00pm ET"
start_iso: "2026-06-05T14:00:00.000Z"
end_iso: "2026-06-05T18:00:00.000Z"
host: "XLAI"
venue_name: "New York"
venue_address: "New York, NY"
neighborhood: "Midtown"
rsvp_url: "https://partiful.com/e/hy44NsXy8fICdvIMp2bI"
public_short_url: "https://go.partiful.com/FU4D2eEJ"
google_maps: "https://www.google.com/maps/search/?api=1&query=New%20York%2C%20New%20York%2C%20NY"
image: "https://firebasestorage.googleapis.com/v0/b/getpartiful.appspot.com/o/external%2Fuser%2F4r1CIfaQQQPa4kMHUpZpu2Kyl2k2%2FTZjJ7qvbGP0nWrVQndMpK"
visibility: "public"
at_capacity: false
going_guest_count: 20
badges: []
fetch_status: ok
owner_count: 2
owner_ids: ["4r1CIfaQQQPa4kMHUpZpu2Kyl2k2", "7DFu4rITofNzKIjA7hCx"]
is_capped: false
total_guest_count: 23
approved_guest_count: 0
local_image: "images/hy44NsXy8fICdvIMp2bI.jpg"
---

# Real-Time Sprint: Build It Live

![Real-Time Sprint: Build It Live](images/hy44NsXy8fICdvIMp2bI.jpg)

**Hosts:** XLAI  _(+1 more host(s) on Partiful — see description for full list)_
**When:** Friday, June 5, 2026 · 10:00am–2:00pm ET
**Where:** New York · New York, NY · Midtown
**Going:** 20 of 23
**RSVP:** https://partiful.com/e/hy44NsXy8fICdvIMp2bI
**Map:** https://www.google.com/maps/search/?api=1&query=New%20York%2C%20New%20York%2C%20NY

### Hosts on Partiful

- [Lyndsay Goldfarb](https://partiful.com/u/4r1CIfaQQQPa4kMHUpZpu2Kyl2k2)
- _(plus Partiful platform admin `7DFu4rITofNzKIjA7hCx` — auto-added to most events)_


## Description

Description

A fast, high-energy morning sprint where builders create and demo real-time experiences in just a few hours.

No slides. No long hackathon. Just show up and build something that happens live.

We’re exploring what the internet feels like when interaction is immediate—voice, chat, AI, collaboration, and presence.

⸻

Time

Friday, June 5 | Morning Session
10:00 AM – 2:00 PM (4 hours)

⸻

Format

* 10:00 – 10:15 | Kickoff + prompt drop
* 10:15 – 12:15 | Build sprint (solo or teams)
* 12:15 – 1:45 | Live demos (2–3 min each)
* 1:45 – 2:00 | Voting, shoutouts, wrap

⸻

What you can build

Anything that feels real-time:

* live AI assistants you can talk to
* voice-first or conversation-based apps
* real-time collaboration tools
* interactive social experiences
* presence-based or “in-the-moment” systems

(Some teams may explore voice, AI, or communication layers using real-time APIs.)

⸻

Why come

You’ll leave with:

* a working prototype in a few hours
* immediate feedback from other builders
* ideas shaped by real-time interaction, not theory

⸻

Core idea

What if everything you built had to happen live?

This event is a part of #NYTechWeek—a week of events hosted by VCs and startups to bring together the tech ecosystem. Learn more at www.tech-week.com






---

[RSVP on Partiful →](https://partiful.com/e/hy44NsXy8fICdvIMp2bI)
