---
title: "AI Ops in Action: Live Demos + Founder Showcases"
event_id: "deH1UugMJh8eLejlukJ5"
date: 2026-06-04
day: "Thursday"
start_time: "5:30pm ET"
end_time: "7:30pm ET"
start_iso: "2026-06-04T21:30:00.000Z"
end_iso: "2026-06-04T23:30:00.000Z"
host: "AutoOpsStack, Brainstation, AI Networking Lab"
venue_name: "BrainStation"
venue_address: "136 Crosby St, New York, NY 10012"
neighborhood: "SoHo"
rsvp_url: "https://partiful.com/e/deH1UugMJh8eLejlukJ5"
public_short_url: "https://go.partiful.com/3YUEDNWO"
google_maps: "https://www.google.com/maps/search/?api=1&query=136%20Crosby%20St%2C%20New%20York%2C%20NY%2010012"
image: "https://firebasestorage.googleapis.com/v0/b/getpartiful.appspot.com/o/external%2Fuser%2Fq4fEYeb9taNZrLrxibJd7zWiXDi1%2FNdMX5v0UkMl1DYHd0yWoT"
visibility: "public"
guest_action: "APPLY"
at_capacity: false
going_guest_count: 91
badges: []
fetch_status: ok
owner_count: 3
owner_ids: ["q4fEYeb9taNZrLrxibJd7zWiXDi1", "7DFu4rITofNzKIjA7hCx", "bNfbgEomzJbCHmrYhIo6ObmYIJF3"]
is_capped: true
max_capacity: 120
remaining_capacity: 35
total_guest_count: 91
approved_guest_count: 85
local_image: "images/deH1UugMJh8eLejlukJ5.png"
---

# AI Ops in Action: Live Demos + Founder Showcases

![AI Ops in Action: Live Demos + Founder Showcases](images/deH1UugMJh8eLejlukJ5.png)

**Hosts:** AutoOpsStack, Brainstation, AI Networking Lab
**When:** Thursday, June 4, 2026 · 5:30pm–7:30pm ET
**Where:** BrainStation · 136 Crosby St, New York, NY 10012 · SoHo
**Capacity:** capped at 120 · 85 taken, 35 remaining
**Access:** apply / hosts approve
**RSVP:** https://partiful.com/e/deH1UugMJh8eLejlukJ5
**Map:** https://www.google.com/maps/search/?api=1&query=136%20Crosby%20St%2C%20New%20York%2C%20NY%2010012

### Hosts on Partiful

- [Neha Panwar](https://partiful.com/u/q4fEYeb9taNZrLrxibJd7zWiXDi1)
- [Nicholas Scibetta](https://partiful.com/u/bNfbgEomzJbCHmrYhIo6ObmYIJF3)
- _(plus Partiful platform admin `7DFu4rITofNzKIjA7hCx` — auto-added to most events)_


## Description

Skip the theory. This evening is built for people who want to see AI, automation, and ops in action — real tools, real teams, real results.

Founder Showcases put builders on stage first. Hear from founders solving real problems across different domains — not just AI companies — alongside the team behind this event, who are building across communities and industries. 

Live Demos follow, showing how builders use AI to solve real problems — across hiring, operations, and beyond. Live products, not pitch decks.

Then we open the floor. The networking session is designed to connect you with founders, operators, and builders working at the intersection of AI, automation, and ops.

If you're curious about how work is evolving — whether you're a founder, operator, or just exploring — this is the room to be in.

What to expect:
Founder showcases of  products across domains
Live demos of AI automation in action
High-signal conversations with builders and operators

This event is co-hosted by AutoOpsStack (AI automation for real-world operations) x BrainStation x AI Networking Lab (Founder led community for learning and applying AI).

BrainStation is the global leader in digital skills learning and workforce transformation. They empower businesses and professionals to succeed in the digital age by equipping them with the necessary skills to create value in fields such as AI, data, design, marketing and product.

This event is a part of #NYTechWeek—a week of events hosted by VCs and startups to bring together the tech ecosystem. Learn more at www.tech-week.com.






---

[Apply on Partiful →](https://partiful.com/e/deH1UugMJh8eLejlukJ5)
