---
title: "Health AI from Research to Production"
event_id: "BSTxVUJmobXv6Y1EYQDX"
date: 2026-06-04
day: "Thursday"
start_time: "2:00pm ET"
end_time: "5:00pm ET"
start_iso: "2026-06-04T18:00:00.000Z"
end_iso: "2026-06-04T21:00:00.000Z"
host: "Scalpels & Spreadsheets, Bright Signal Consulting"
venue_name: "307 West 38th Street"
venue_address: "307 W 38th St, New York, NY 10018"
neighborhood: "Midtown"
rsvp_url: "https://partiful.com/e/BSTxVUJmobXv6Y1EYQDX"
public_short_url: "https://go.partiful.com/ApA5ThBo"
google_maps: "https://www.google.com/maps/search/?api=1&query=307%20W%2038th%20St%2C%20New%20York%2C%20NY%2010018"
image: "https://firebasestorage.googleapis.com/v0/b/getpartiful.appspot.com/o/external%2Fuser%2FUGVoirQYOuZcznIiuuU3avFfcTK2%2FeegVmN2RS4st7LTjaFaTE"
visibility: "public"
guest_action: "APPLY"
at_capacity: false
going_guest_count: 166
badges: []
fetch_status: ok
owner_count: 4
owner_ids: ["9K2ip5KfG1VOTKg73Vjq", "7DFu4rITofNzKIjA7hCx", "UGVoirQYOuZcznIiuuU3avFfcTK2", "GNdSkZC3mzUVf88s2xjJuHqp98w1"]
is_capped: true
max_capacity: 140
remaining_capacity: 27
total_guest_count: 166
approved_guest_count: 113
local_image: "images/BSTxVUJmobXv6Y1EYQDX.png"
---

# Health AI from Research to Production

![Health AI from Research to Production](images/BSTxVUJmobXv6Y1EYQDX.png)

**Hosts:** Scalpels & Spreadsheets, Bright Signal Consulting  _(+2 more host(s) on Partiful — see description for full list)_
**When:** Thursday, June 4, 2026 · 2:00pm–5:00pm ET
**Where:** 307 West 38th Street · 307 W 38th St, New York, NY 10018 · Midtown
**Capacity:** capped at 140 · 113 taken, 27 remaining
**Access:** apply / hosts approve
**RSVP:** https://partiful.com/e/BSTxVUJmobXv6Y1EYQDX
**Map:** https://www.google.com/maps/search/?api=1&query=307%20W%2038th%20St%2C%20New%20York%2C%20NY%2010018

### Hosts on Partiful

- [Scalpels & Spreadsheets](https://partiful.com/u/9K2ip5KfG1VOTKg73Vjq) — _North America's fastest growing healthcare community_
- [Brittany Sigler](https://partiful.com/u/UGVoirQYOuZcznIiuuU3avFfcTK2)
- [Alexander Singh](https://partiful.com/u/GNdSkZC3mzUVf88s2xjJuHqp98w1) — _healthcare x ai_
- _(plus Partiful platform admin `7DFu4rITofNzKIjA7hCx` — auto-added to most events)_


## Description

*Light bites and beverages will be served thanks to our friends at R1!*

Most health AI events follow the same formula: researchers talk about what they built. Founders talk about what they're selling. The audience leaves wondering how the two connect.

We want to break that. We're bringing together scientist-founders to show how the worlds of AI research and healthcare product converge. How do you take frontier science—whether that's foundation models, behavioral insights, or clinical validation—and turn it into a product that clinicians and patients actually use? What do you keep? What do you cut? Where does rigor meet velocity?

For founders scaling health AI, for investors evaluating science vs. hype, for researchers building for impact: this is the hour where the real work gets named.

We're kicking off with a fire-side chat with Martin Seneviratne (R1) for a fireside chat on moving healthcare AI from theory to operation. What does it take to deploy AI in environments where accuracy, compliance, and scale all matter at once? We’ll unpack how R1 is turning messy clinical and financial data into working systems—and what that means for builders who want to take AI beyond demos and into tools people depend on every day.

Next we'll dig into a panel with early-stage founders, living these challenges on a daily basis.

Speakers: 
Fireside: Martin Seneviratne, Co-CEO of R37 @ R1

Panel:
Alexander Singh, Associate, Yosemite
Vitalay Fomin, Co-Founder/CEO @ Numenos AI
Nyamitse-Calvin Mahinda, Founder/CEO @ Vital Audio

Moderator: Brittany Sigler, Founder/Fractional Product Exec @ Bright Signal Consulting

Agenda:
Doors Open: 2.00PM
Networking:  2.00 - 3.00PM
Panel: 3.00PM - 3.50PM
Networking: 3.50 - 5.00PM 

This event is a part of #NYTechWeek—a week of events hosted by VCs and startups to bring together the tech ecosystem. Learn more at www.tech-week.com.






---

[Apply on Partiful →](https://partiful.com/e/BSTxVUJmobXv6Y1EYQDX)
