---
title: "Founders x Investors AI Toolkit & Happy Hour"
event_id: "Vs2eC7nERtgF0PgNWfUo"
date: 2026-06-02
day: "Tuesday"
start_time: "6:00pm ET"
end_time: "9:00pm ET"
start_iso: "2026-06-02T22:00:00.000Z"
end_iso: "2026-06-03T01:00:00.000Z"
host: "Hatsu"
venue_name: "New York, NY"
venue_address: "New York, NY"
neighborhood: "Midtown"
rsvp_url: "https://partiful.com/e/Vs2eC7nERtgF0PgNWfUo"
public_short_url: "https://go.partiful.com/84DqAseV"
google_maps: "https://www.google.com/maps/search/?api=1&query=New%20York%2C%20NY%2C%20New%20York%2C%20NY"
image: "https://firebasestorage.googleapis.com/v0/b/getpartiful.appspot.com/o/external%2Fuser%2FYFnPMvlFrcfWjtE6PBVqVX8udC73%2F32SRwImXG_fO_OXToOz5k"
visibility: "public"
guest_action: "APPLY"
at_capacity: false
going_guest_count: 37
badges: []
fetch_status: ok
owner_count: 3
owner_ids: ["YFnPMvlFrcfWjtE6PBVqVX8udC73", "7DFu4rITofNzKIjA7hCx", "7zRUtTpKxyIDK9UN0Sim"]
is_capped: false
total_guest_count: 37
approved_guest_count: 28
local_image: "images/Vs2eC7nERtgF0PgNWfUo.png"
---

# Founders x Investors AI Toolkit & Happy Hour

![Founders x Investors AI Toolkit & Happy Hour](images/Vs2eC7nERtgF0PgNWfUo.png)

**Hosts:** Hatsu  _(+2 more host(s) on Partiful — see description for full list)_
**When:** Tuesday, June 2, 2026 · 6:00pm–9:00pm ET
**Where:** New York, NY · New York, NY · Midtown
**Going:** 37
**Access:** apply / hosts approve
**RSVP:** https://partiful.com/e/Vs2eC7nERtgF0PgNWfUo
**Map:** https://www.google.com/maps/search/?api=1&query=New%20York%2C%20NY%2C%20New%20York%2C%20NY

### Hosts on Partiful

- [Hatsu Life](https://partiful.com/u/YFnPMvlFrcfWjtE6PBVqVX8udC73) — _Hatsu - Campaigns that launch themselves! 

https://hatsu.life_
- [Hatsu](https://partiful.com/u/7zRUtTpKxyIDK9UN0Sim) — _Campaigns that build themselves 

https://hatsu.life_
- _(plus Partiful platform admin `7DFu4rITofNzKIjA7hCx` — auto-added to most events)_


## Description

Join founders, investors, and AI builders for Founders x Investors: AI Toolkit—an evening exploring how AI is transforming product management, financial services, and agentic networks.

The event marks the global debut of Hatsu, an interactive experiential activation where guests can step into the Hatsu Studio and watch their next marketing campaign create itself in real time. 

Bring an idea, prompt, product, or brand challenge—and leave with a fully generated campaign concept, creative assets, and the chance to win prizes.

Expect panel discussions, networking, happy hour with AI robots bartenders, and a first look at the future of AI-powered marketing creation.

Learn more about Hatsu: https://hatsu.life

This event is a part of #NYTechWeek—a week of events hosted by VCs and startups to bring together the tech ecosystem. Learn more at www.tech-week.com






---

[Apply on Partiful →](https://partiful.com/e/Vs2eC7nERtgF0PgNWfUo)
